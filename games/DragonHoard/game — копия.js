...
const GAME_CONSTANTS = {
    ...
    ENEMY_SPAWN_DELAY: 2000,        // Задержка между спавном врагов (2 секунды)
    ...
    TREASURE_HP_LOSS_RATE: 1000,    // Частота уменьшения здоровья сокровища (1 секунда)
...
let gameWidth, gameHeight, uiElements, gameOverText, gameOverRect;
...
function update() {
    dragon.move(cursors); // Управление драконом
...
function spawnBonus(x, y) {
    ...
    if (bonus) {
        bonus.setActive(true).setVisible(true)
            .setVelocityY(100)
            .setData('isWaveBonus', true); // Оставляем как бонус с эффектом восстановления
    }
}
