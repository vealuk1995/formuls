    ...
    shoot() {
        ...
        if (fireball) {
            ...
            if (this.lastDirection === -1) {
                fireball.body.setOffset(36, 0); // Смещаем тело влево при движении влево
            } else {
                fireball.body.setOffset(0, 0); // Нормальная позиция при движении вправо
            }
...
function update() {
    ...
    enemies.getChildren().forEach(enemy => {
        ...
        if (hasCoin) {
            ...
            // Корректируем смещение тела
            if (spawnSide === 'left') {
                enemy.body.setOffset(48, 0); // Влево
            } else {
                enemy.body.setOffset(0, 0); // Вправо
            }
        } else {
