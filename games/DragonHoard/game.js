...
function create() {
    ...
    if (!this.plugins.get('rexVirtualJoystick')) {
        console.warn('Virtual Joystick plugin not found');
    }

    // Создание анимаций один раз при первом запуске сцены
    ...
    startScreen.add(this.add.image(gameWidth / 2, gameHeight / 2, 'cave_bg').setDisplaySize(gameWidth, gameHeight));
    startScreen.add(this.add.text(gameWidth / 2, gameHeight / 2 - 50, 'Dragon\'s Hoard', { fontFamily: 'MedievalSharp', fontSize: '60px', color: '#ffd700' }).setOrigin(0.5));
    startScreen.add(this.add.text(gameWidth / 2, gameHeight / 2 + 50, 'Press ENTER or Tap to Start', { fontFamily: 'MedievalSharp', fontSize: '30px', color: '#fff' }).setOrigin(0.5));

    this.input.keyboard.once('keydown-ENTER', () => startGame.call(this), this);
    if (isMobileDevice()) {
        this.input.once('pointerdown', () => startGame.call(this), this);
    }
...
function startGame() {
    ...
    uiElements = {
        hpText: scene.add.text(gameWidth / 2 - 50, gameHeight * GAME_CONSTANTS.UI_HP_Y, `Treasure HP: ${treasureHealth}`, { fontSize: '20px', color: '#fff' }),
        scoreText: scene.add.text(gameWidth * GAME_CONSTANTS.UI_SCORE_X, gameHeight * 0.033, 'Score: 0', { fontSize: '20px', color: '#fff' }),
        dragonHpText: scene.add.text(gameWidth * GAME_CONSTANTS.UI_DRAGON_HP_X - 50, gameHeight * 0.033, `Dragon HP: ${GAME_CONSTANTS.DRAGON_MAX_HEALTH}`, { fontSize: '20px', color: '#fff' }),
        waveText: scene.add.text(gameWidth / 2, gameHeight * GAME_CONSTANTS.UI_WAVE_Y, '', { fontSize: '25px', color: '#ffd700' }).setOrigin(0.5)
    };
