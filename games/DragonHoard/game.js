...
const config = {
    type: Phaser.AUTO, // Автоматический выбор рендера (WebGL или Canvas)
    parent: 'game-container', // ID HTML-элемента, в котором будет размещена игра
    width: 1280, // Ширина игрового холста в пикселях
    height: 720, // Высота игрового холста в пикселях
    scale: {
        mode: Phaser.Scale.FIT, // Режим масштабирования: подгонка под размер экрана с сохранением пропорций
        autoCenter: Phaser.Scale.CENTER_BOTH // Центрирование игры по горизонтали и вертикали
    },
    physics: {
        default: 'arcade', // Используется физическая система Arcade (простая и быстрая)
        arcade: {
            gravity: { y: GAME_CONSTANTS.GRAVITY_Y }, // Установка гравитации из констант
            debug: true // Включение отладки физики (показывает хитбоксы и траектории)
        }
    },
    scene: { preload, create, update } // Основные функции сцены: загрузка, создание и обновление
};

// Создание экземпляра игры с заданной конфигурацией
const game = new Phaser.Game(config);

// Глобальные переменные для управления объектами и состоянием игры
let dragon, treasure, platforms, enemies, fireballs, bonuses, cursors; // Основные игровые объекты
let score = 0, treasureHealth = GAME_CONSTANTS.INITIAL_TREASURE_HP, waveCount = 0; // Счет, здоровье сокровища и номер волны
let gameWidth, gameHeight, uiElements, gameOverText, gameOverRect, startScreenText, startScreenRect, progressBar, progressScroll, startButton; // Переменные для UI и размеров
let isGameStarted = false, isWaveInProgress = false; // Флаги состояния игры
let debugRects = []; // Массив для хранения отладочных прямоугольников

// Класс дракона, управляющий его поведением и свойствами
class Dragon {
    constructor(scene, x, y, fireballs) {
        this.scene = scene; // Ссылка на текущую сцену Phaser
        this.fireballs = fireballs; // Группа огненных шаров для стрельбы
...
function create() {
    ...
    startButton.on('pointerdown', () => window.startGame(this)); // Делаем startGame глобальной функцией
    ...
    this.input.keyboard.on('keydown-ENTER', () => window.startGame(this));
...
// Экспортируем startGame как глобальную функцию для доступа из mobile.js
window.startGame = function(scene) {
    if (isGameStarted || !scene.load.isReady()) return;
    isGameStarted = true;
